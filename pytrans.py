...
class PyTrans(object):
    def __init__(self, py_hanzi):
        """
		根据下面的元组建立字典
		"""
        self.hanpin = {}  #汉字(str) —> 拼音(list)
        for pinyin, hanzi in py_hanzi:
            hanzi = [i for i in hanzi]
            for char in hanzi:
                self.hanpin.setdefault(ord(char), []).append(pinyin)  #setdefault方法，非常有用
    ...
    def h_to_p(self, words):
        ...
        for char in words:
            ord_char = ord(char)
            if ord_char in self.hanpin:
                pinyin.append(self.hanpin[ord_char])
            else:
                pinyin.append(list(char))
        #return list(self._perm(pinyin)) #列表形式
        return [''.join(i) for i in list(self._perm(pinyin))]  #字符串形式

    def p_to_h(self, pinyin):
        pass
...
py_hanzi = [
    ...
    ("de", "的得锝德地"),
    ("deng", "邓灯登澄蹬瞪嶝凳噔簦镫磴戥等"),
    ("di", "帝蒂谛缔碲弟绨娣递睇第地氐低底抵坻诋砥柢骶羝堤提迪滴嘀镝嫡敌邸狄荻棣笛涤的嶂觌籴"),
    ("dian", "店掂惦踮点坫玷电垫碘典淀靛殿癜甸滇巅颠癫奠佃钿簟皲堤岸"),
    ("diao", "调雕凋鲷碉掉刁叼吊铞钓貂铫鸟"),
    ("die", "谍蹀碟蝶牒鲽喋揲堞跌叠迭耋瓞垤"),
    ("ding", "定锭腚碇啶丁订盯钉仃叮町酊玎耵顶疔鼎铤"),
    ("diu", "丢铥"),
    ("dong", "动东栋冻胨岽鸫洞胴峒硐侗恫垌冬咚氡董懂"),
    ("dou", "都斗抖蚪豆逗痘兜篼蔸陡窦读"),
    ("du", "渎读牍犊椟黩度渡镀杜肚毒独睹赌堵都嘟嘟督笃蠹妒黢髑顿芏"),
    ("duan", "段锻缎煅椴断簖短端"),
    ("dui", "对怼队堆碓兑敦镦憝"),
    ("dun", "吨顿钝盹炖砘沌囤敦墩礅镦盾遁蹲趸"),
    ("duo", "多哆夺驮朵垛躲跺铎剁哚舵柁度踱咄堕惰掇裰缍沲"),
    ("e", "锷鄂鳄鹗萼谔腭愕颚额俄饿娥峨鹅蛾哦锇莪阿婀屙恶噩垩厄扼苊轭呃阏遏讹"),
    ("en", "恩摁蒽"),
    ("er", "二而鸸鲕尔迩儿耳洱饵铒珥佴贰"),
    ("fa", "发法砝珐伐阀筏垡乏罚"),
    ("fan", "反贩饭返畈凡帆矾钒梵番翻藩幡燔蹯蕃范犯樊繁蘩泛烦"),
    ("fang", "方放房防芳访纺仿坊邡舫枋妨钫肪鲂钫彷"),
    ("fei", "费沸狒镄狒飞废肥淝非菲啡扉诽腓痱榧悱鲱绯匪篚斐翡蜚霏吠妃芾肺"),
    ("fen", "分份粉汾纷酚玢吩鼢芬棼忿氛坟奋态粪愤鲼偾焚瀵"),
    ("feng", "风疯讽砜枫封葑丰沣冯峰锋蜂烽逢缝奉俸唪凤酆"),
    ("fo", "佛"),
    ("fou", "否缶"),
    ("fu", "富副福幅辐蝠匐抚赴讣夫扶肤蚨呋芙趺麸蚨负复蝮腹鳆馥覆服付府附俯鲋驸跗符苻孚莩浮俘郛桴咐拊腑蜉孵腐稃阜妇伏袱茯父斧釜滏甫傅辅脯黼赙缚敷弗佛拂砩怫绋艴氟绂绂祓黻黻赋涪罘芾疠凫菔幞"),
    ("ga", "嘎噶旮伽咖胳轧钆尕尜尬夹"),
    ("gai", "该改盖丐钙概溉赅陔垓芥戤"),
    ...
    ("guang", "广犷光咣胱桄逛"),
    ("gui", "妫贵归轨圭桂硅鲑闺鬼傀瑰龟柜规炔炅跪诡癸獬庋宄匦刿刽簋桧晷皈鳜"),
    ("gun", "衮滚磙棍辊绲鲧"),
    ...
    ("ji",
     "几机饥矶叽肌玑讥虮及圾极圾汲级芨笈岌己记纪鸡基计集际齐济挤剂跻荠霁哜鲚吉佶诘咭髻继系急积季悸冀籍藉技伎妓芰屐击暨绩姬箕稽蓟给寄奇畸犄掎迹疾蒺忌跽激祭辑缉楫骥寂脊瘠嵴期稷棘鲫嵇羁麂戟戢蕺革其亟殛洎觊笄齑偈赍即既唧乩丌墼剞畿嫉"),
    ("jia", "家嫁稼镓嘉价加伽珈枷跏架驾笳假茄痂袈迦嘏瘕葭佳甲钾胛岬贾夹郏蛱铗浃颊荚戛袷恝"),
    ("jian", "件建健键腱踺毽楗犍间涧锏裥简见枧舰笕减碱缄兼缣搛蒹鹣检剑捡俭硷睑肩监槛尖坚菅蹇渐鉴剪翦箭湔煎谫拣歼茧荐鞯柬谏戋浅饯贱践溅笺奸艰謇戬僭鲣囝趼牮"),
    ...
    ("jing", "竞竟境镜獍泾经劲刭颈径弪茎迳痉井阱肼精靖腈睛婧靓静菁净景憬京惊鲸敬警荆晶兢儆旌粳"),
    ("jiong", "窘迥炯扃炅"),
    ("jiu", "就僦蹴纠赳九旧酒久疚玖救究柩灸揪啾鬏臼桕舅咎鹫鸠厩韭阄"),
    ("ju", "局居据椐踞倨琚裾锯巨拒距炬矩柜钜讵苣榘句驹拘枸锔具俱犋惧飓举榉剧聚菊鞠掬且咀雎龃沮趄狙苴疽遽踽瞿醵莒屦窭橘鞫桔车"),
    ...
    ("leng", "愣楞塄棱冷"),
    ("li", "丽鹂郦骊俪鲡逦厉疠粝励呖雳砺蛎力历沥坜枥苈疬砾栎轹跞里理哩喱狸鲤锂俚悝厘娌李利俐蜊猁梨犁莉痢立粒莅笠离缡漓璃篱蓠礼黎藜例栗溧傈篥荔澧醴鳢罹蠡吏隶鬲戾唳嫠詈黧"),
    ...
    ("qi", "奇骑绮崎琦欹其期旗棋琪祺淇麒骐蜞欺萁葺七气汽齐蛴脐荠企器启岐妻萋凄弃蕲祁乞汔迄讫亓祈圻漆戚槭嘁杞屺岂桤芑起砌沏歧栖泣契芪憩耆鳍畦稽蹊碛綦俟颀綮缉亟柒"),
    ("qia", "卡恰洽掐髂葜"),
    ...
    ("shi", "是市柿铈师饰十士仕什使时鲥埘莳事式拭试轼弑石炻史驶室世视实诗峙侍恃施氏势始食失识狮示湿适释尸拾似逝誓蚀矢屎螫嗜蓍筮噬虱鲺匙谥贳酾殖嘘豕舐"),
    ("shou", "手收首售守狩寿受授瘦绶兽艏熟"),
    ...
    ("wei", "为伪沩维唯惟潍位未味委诿逶萎痿微薇威葳崴卫尾娓艉危魏巍嵬韦伟纬玮炜帏苇违韪围涠闱尉慰蔚畏猥隗喂偎煨隈胃谓渭猬圩帷桅遗鲔洧"),
    ("wen", "问文纹蚊汶雯温瘟闻稳吻刎璺紊阌"),
    ("weng", "翁瓮嗡蓊蕹"),
    ("wo", "我硪涡蜗窝莴握渥龌喔幄沃卧肟挝斡倭"),
    ("wu", "五伍吾悟捂梧晤唔牾焐浯鼯痦寤无怃妩芜庑吴误蜈乌邬坞钨呜鹜芜勿物芴务雾婺武鹉午仵忤迕屋舞污圬毋侮巫诬恶戊兀杌阢鋈於"),
    ("xi", "西硒栖牺茜粞舾系奚溪蹊喜禧嬉嘻僖熹戏息熄媳螅洗细锡裼希稀烯唏浠郗席习吸隰袭舄析晰淅蜥皙菥熙昔惜腊悉夕矽汐穸兮膝玺歙曦羲隙犀铣徙屣阋禊翕醯蟋檄觋欷葸樨蓰饩鼹鼷"),
    ...
    ("yan", "严俨酽砚言唁盐焰阎眼烟咽胭燕演验艳滟岩延蜒筵沿铅炎琰剡焱研妍彦谚晏鼹宴堰郾偃颜焉鄢嫣雁奄腌淹掩崦厣罨阉衍闫厌恹檐赝湮殷魇餍兖芫鼯阽谳阏菸"),
    ("yang", "阳养羊样洋蛘烊徉佯氧痒漾恙央秧泱怏鞅殃鸯杨扬炀疡仰"),
    ("yao", "要腰药姚珧铫耀曜幺么吆窈尧侥瑶摇谣遥鳐崾徭鹞繇窑陶咬邀夭妖杳舀钥约爻肴轺疟"),
    ("ye", "也业叶夜页液掖腋野邺冶爷烨晔哗耶揶椰邪铘噎曳谒靥咽"),
    ("yi",
     "一以苡已异乙亿忆钇屹仡艺呓易埸蜴意臆镱噫薏癔荑痍亦益溢镒嗌缢依铱医宜谊义仪议蚁舣艾衣裔伊咿移移羿弈毅译驿怿峄绎弋奕翼疑嶷沂遗逸役疫邑挹悒轶佚矣怡眙诒贻饴夷姨胰咦熠抑椅猗倚漪旖欹彝壹殪颐尾迤翌翊黟懿诣揖佾劓刈肄酏翳蛾瘗圯蛇"),
    ...
    ("yu", "与屿欤于吁纡宇芋竽迂盂余馀狳玉钰鱼渔域雨雩育誉予妤豫预蓣语龉圄谷裕浴峪欲鹆俞喻蝓愉渝榆瑜揄谕觎嵛愈逾窬毓郁禹俣娱虞羽禺喁隅愚遇寓御狱於淤瘀昱煜舆尉蔚熨聿鹬臾谀腴萸庾瘐燠驭窳菀蜮妪伛圉鬻阈舁饫畲粥"),
    ...
    ("zhi", "至侄桎轾蛭致窒膣之芝只识职织帜轵枳制志支枝忮吱肢治直值植殖埴置质踬纸旨酯指脂知蜘智执止址趾祉芷滞郅汁掷踯炙稚峙痔雉挚秩栉痣彘骘贽鸷帙黹豸跖絷徵摭咫祗胝觯卮栀瘛氏"),
...
pytrans = PyTrans(py_hanzi)
"""
为其他模块的可复用创建一个简单唯一接口pytrans
这样，在其他模块引用本模块的时候，只需要：
-------other.py--------
from pytrans import pytrans

l = pytrans.h_to_p("辣椒面12138")
-----------------------
"""

if __name__ == '__main__':
    l = pytrans.h_to_p("岑科廷12138")
    print(l)
